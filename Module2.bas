...
If forwarder = "DSV" Then
    ...
    Dim truckLabel As String

    Dim currentTime As Date: currentTime = Now

    Dim todayWeekday As Integer: todayWeekday = Weekday(currentTime, vbMonday)

    Dim isWeekend As Boolean: isWeekend = (todayWeekday > 5)

    Dim isAfter1630 As Boolean: isAfter1630 = (TimeValue(currentTime) >= TimeValue("16:30:00"))

    If priority = "Emergency" And (isAfter1630 Or isWeekend) Then
    ...
    ElseIf TimeValue(currentTime) < TimeValue("16:30:00") Then
    ...
    Else
        ...
        truckLabel = "Truck 1 " & Format(nextDay, "dd/mm")
